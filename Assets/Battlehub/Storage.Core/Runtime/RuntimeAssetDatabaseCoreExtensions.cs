...
namespace Battlehub.Storage
{
    public static class RuntimeAssetDatabaseCoreExtensions
    {
        ...
        public static IReadOnlyList<IMeta<TID, TFID>> GetChildrenMeta<TID, TFID>(this IAssetDatabase<TID, TFID> assetDatabase, TID parentID, bool recursive = false)
          where TID : IEquatable<TID>
          where TFID : IEquatable<TFID>
        {
            var idList = new List<TID>();
            GetChildren(assetDatabase, parentID, recursive, includeFolders: true, includeAssets: true, idList);
            
            var metaList = new List<IMeta<TID, TFID>>(idList.Count);
            foreach (var id in idList)
            {
                metaList.Add(assetDatabase.GetMeta(id));
            }
            return metaList;
        }

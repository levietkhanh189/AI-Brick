...
namespace Battlehub.Storage
{
    public interface ISurrogatesGen
    {
        ...
        IEnumerable<MemberInfo> GetEnumerableProperties(Type type);
    ...
    {
        private const string k_surrogateTemplate = @"using ProtoBuf;
using System;
...
";
        ...
        private const string k_valueTypeSurrogateTemplate = @"using ProtoBuf;
using System;
...
";
        ...
        private const string k_beginDeserializationNoParameterlessCtorTemplate = @"
            ...
            if (o == null)
            {{
                #warning There is no parameterless constructor. If necessary, you can add code here to create an instance of {0}.
...
{3}
        ...
        public string GetSurrogateCode(Type type, int typeIndex)
        {
            ...
            string surrogateNamespace;
            
            if (type.IsNested)
            {
                //Nested class surrogates use the full name of the parent class as a namespace
                surrogateNamespace = $".{type.DeclaringType.FullName.Replace(" +", ".")}";
            }
            else
            {
               surrogateNamespace = !string.IsNullOrEmpty(type.Namespace) ?
                            $".{type.Namespace}" :
                            string.Empty;
            }
        ...
        {
            var properties = GetEnumerableProperties(type);
        ...
        {
            var allSerializableProperties = GetSerializableProperties(type);
            KeyValuePair<string, MemberInfo>[] serializableProperties = allSerializableProperties
                .Select(p => ($"{GetSurrogateTypeName(GetPropertyType(p))} {p.Name}", p))
        ...
        {
            var result = GetSerializableMembers(type, includeProperties:true);
        ...
        public IEnumerable<MemberInfo> GetSerializableMembers(Type type, bool includeProperties)
        {
            return m_config.GetSerializableMembers(type, includeProperties);
        }

        public IEnumerable<MemberInfo> GetEnumerableProperties(Type type)
        {
            return m_config.GetEnumerableProperties(type);
        }
        ...
        private string PropertyFullName(MemberInfo memberInfo)
        {
            ...
            static string GetTypeFullName(Type type)
            {
                if (type.IsGenericType)
                {
                    ...
                    string genericTypeName = $"global::{type.Namespace}.{type.Name.Replace($"`{args.Length}", "")}<";
        ...
        private string GetSurrogateTypeName(Type propertyType)
        {
            ...
            return propertyTypeName.Replace("+", ".");
        }
        ...
        public IEnumerable<Type> GetElementTypes(Type type)
        {
            ...
            else if (IsSerializableArrayOrList(type))
            {
                yield return type.GetGenericArguments().Single();
            }
        }
        ...
        private void GetDependencies(Type type, HashSet<Type> dependencies, bool recursive)
        {
            foreach (MemberInfo memberInfo in GetSerializableProperties(type))
            {
                ...
                if (propertyType.IsEnum || TypeFinder.PrimitiveTypes.Contains(propertyType) || propertyType.IsPrimitive || !m_config.IsSerializableType(propertyType))
                {
                    continue;
                }
        ...
        private bool IsSerializableArrayOrList(Type type)
        {
            return type.IsGenericType && !string.IsNullOrEmpty(type.FullName) && (
                type.FullName.StartsWith("Battlehub.Storage.SerializableList") ||
                type.FullName.StartsWith("Battlehub.Storage.SerializableArray"));
        }

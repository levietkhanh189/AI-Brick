...
namespace Battlehub.Storage
{
    ...
    public class AssetLoaderAdapter<TID> : IExternalAssetLoader, IExternalIDMap<TID>
    {
        ...
        private readonly IExternalAssetLoader m_loaderImpl;
        private readonly IExternalIDMap<TID> m_externalIDMap;
        public AssetLoaderAdapter(IExternalAssetLoader impl)
        {
            m_loaderImpl = impl;
            m_externalIDMap = impl as IExternalIDMap<TID>;
        }
        ...
        public async Task<object> LoadAsync(string key, object root, IProgress<float> progress = null)
        {
            ...
            {
                obj = await m_loaderImpl.LoadAsync(key, root, progress);
                if (obj == null)
                {
                    return null;
                }
        ...
        public void Release(object obj)
        {
            if (m_objectToKey.TryGetValue(obj, out var key))
            {
                ...
                if (refCounter == 0)
                {
                    ...
                    m_loaderImpl.Release(externalAsset);
        ...
        {
            if (m_externalIDMap != null)
            {
                return m_externalIDMap.TryGetID(obj, out id);
            }

    ...
    public static class IIDMapExtensions
    {
        ...
        public static T GetComponent<T, TID>(this IIDMap<TID> idmap, TID componentID, TID gameObjectID, Transform tempRoot = null) where TID : IEquatable<TID> where T : Component
        {
            ...
            if (component == null)
            {
                ...
                if (component == null)
                {
                    // The change propagation mechanism is designed in such a way that additions are propagated first, and then deletions
                    // This is the fix for the case where components such as AudioListener cannot be added twice when applying
        ...
        public static Component GetComponent<TID>(this IIDMap<TID> idmap, Type componentType, TID componentID, TID gameObjectID) where TID : IEquatable<TID> 
        {
            Debug.Assert(!idmap.NullID.Equals(componentID));

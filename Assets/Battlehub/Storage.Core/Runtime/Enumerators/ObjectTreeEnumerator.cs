...
namespace Battlehub.Storage
{
    public interface IObjectTreeEnumerator : IEnumerator
    {
        ...
        void SetFilter(Func<object, bool> filter);
    ...
    {
        public static Func<object, bool> IsNullOrDestroyedFunc = obj =>
        {
        ...
        {
            return IsNullOrDestroyedFunc(obj);
        }

        public static Func<object, bool> IsHiddenFunc = obj =>
        {
            if (obj is UnityEngine.Object)
            {
                var uo = (UnityEngine.Object)obj;
                return uo.hideFlags == HideFlags.DontSave;
            }
            return false;
        };
    ...
    public class ObjectTreeEnumerator : IObjectTreeEnumerator
    {
        ...
        private Func<object, bool> m_filter;
        ...
        private static bool DefaultFilter(object obj)
        {
            if (UnityObjectUtils.IsNullOrDestroyed(obj))
            {
                return false;
            }

            if (UnityObjectUtils.IsHidden(obj))
            {
                return false;
            }
        ...
        public bool MoveNext()
        {
            do
            {
                if (m_currentEnumerator.MoveNext())
                {
                    ...
                    if (current != m_currentEnumerator.Object)
                    {
